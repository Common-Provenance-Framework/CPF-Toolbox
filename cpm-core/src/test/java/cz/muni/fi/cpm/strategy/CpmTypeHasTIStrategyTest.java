...
class CpmTypeHasTIStrategyTest {
  ...
  @Test
  public void belongsToTraversalInformation_withCpmTypeAndOtherQNType_returnsTrue() {
    QualifiedName validQualifiedName = pF.newQualifiedName(
        "uri",
        "local",
        "ex");
  ...
  @Test
  public void belongsToTraversalInformation_withCpmTypeAndOtherStringType_returnsTrue() {

    QualifiedName id = pF.newQualifiedName("uri", "entity", "ex");
    Element element = pF.newEntity(id);
    element.getType().add(cPF.newCpmType(CpmType.BACKWARD_CONNECTOR));
    element.getType().add(pF.newType("local", pF.getName().XSD_STRING));
